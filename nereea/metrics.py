...
def get_inconsistencies(nest, nests):
    ...
    for n in nests:
        ...
        if (current_start > start) & (current_end > end):
            break
        elif ((current_start >= start) & (current_start <= end)) | (
    ...
    for e in entities:
        ...
        if (start > last) | (last == 0):
            ...
            for e2 in entities:
                ...
                if ((current_start >= start) & (current_start <= end)) | (
                    (current_end >= start) & (current_end <= end)
                ):
                    if not e2 in nest:
                        nest.append(e2)
...
def check_wrong_label_overlapping_span(entity, inconsistencies):
    ...
    for i in inconsistencies:
        ...
        if (
            (
                len(
                    set(range(start, end + 1)).intersection(
                        range(current_start, current_end + 1)
                    )
                )
                > 0
            )
...
def ner_report(y_true, y_pred):
    ...
    for t in true_nested_entities:
        ...
        t_ = t[:]
        for t2 in t_:
            current_false_negative = check_false(t2, inconsistencies)
            if current_false_negative:
                ...
                t.pop(t.index(t2))
    rest = (
        set(flatten(pred_nested_entities))
        - set(correct)
        - set(false_negative)
        - set(flatten(wrong_label_right_span))
        - set(flatten(wrong_label_overlapping_span))
        - set(flatten(right_label_overlapping_span))
    )
    for r in list(rest):
            ...
            for line in f:
                ...
                if line != "":
                    ...
                    annotations[entity_name].append(
                        {"sentence": sentence, "true": true, "predicted": predicted}
                    )
                    sentence = []
                    true = []
                    predicted = []
        ...
        if i == 0:
            ...
            for j, s in enumerate(val):
                for k in range(len(s["sentence"])):
                    ...
                    current_predicted_annotation = annotations_consolidated[j][
                        "predicted"
                    ][k]
                    ...
                    if new_predicted_annotation[0] == "O":
                        ...
                        current_predicted_annotation.extend(new_predicted_annotation)
    y_trues = [
        annotations_consolidated[i]["true"]
        for i in range(len(annotations_consolidated))
    ]
    y_predicteds = [
        annotations_consolidated[i]["predicted"]
        for i in range(len(annotations_consolidated))
    ]
    sentences = [
        annotations_consolidated[i]["sentence"]
        for i in range(len(annotations_consolidated))
    ]
...
def get_error_pair_text_list(error_pairs, sentences):
    return [
        (
            e[0][0],
            get_text_span(e[0], sentences),
            e[1][0],
            get_text_span(e[1], sentences),
        )
        for e in error_pairs
    ]
